...
class ApiClient extends GetxService {
  ...
  Map<String, String> _prepareFinalHeaders(Map<String, String>? customHeaders) {
    ...
    if (Get.isRegistered<PusherService>()) {
      final String? sId = Get.find<PusherService>().socketId;
      if (sId != null && sId.isNotEmpty) {
        finalHeaders['X-Socket-ID'] = sId;
      }
    }
  ...
  Future<Response<dynamic>?> _ensureHomeHeadersOrBlock(
      String uri, Map<String, String> headers,
      {required String method, bool omitModuleId = false}) async {
    if (!_isHomeOrStoreApi(uri)) return null;
    bool syncAttempted = false;

    _hydrateHomeHeadersFromAppContext(uri, headers);
    if (hasValidHomeHeaders(headers: headers) || omitModuleId) {
      return null;
    }
  ...
  Response<dynamic>? _blockIfInvalidHomeHeaders(
    ...
    if (omitModuleId && _isValidZoneHeaderValue(zoneId)) return null;
    if (kDebugMode) {
      debugPrint(
