...
Future<void> main() async {
  ...
  // âڑ، Background handler registration moved to _initializeHeavyServices()
  ...
  FlutterError.onError = (FlutterErrorDetails details) {
    ...
    if (kDebugMode && details.exception.toString().contains('overflowed')) {
...
/// âڑ، NEW: Initialize ONLY what's needed for routing decision
...
/// Initialize heavy services in background (non-blocking)
/// This runs AFTER the first frame is painted
Future<void> _initializeHeavyServices() async {
  if (_heavyServicesInitialized) {
    appLogger.info('ℹ️ Heavy services already initialized, skipping');
    return;
  }
  ...
  try {
    ...
    if (kDebugMode) {
      debugPrint('✅ Notification service init queued (non-blocking Stage 2)');
    }
...
/// Initialize non-critical services (fire and forget)
...
void _initializeNonCriticalServices() {
  ...
  Future.microtask(() async {
    try {
      ...
      if (kDebugMode) debugPrint('✅ Date formatting already initialized');
    } catch (e) {
  ...
  Widget _buildMaterialApp({
    ...
    return GetMaterialApp(
      ...
      routingCallback: (routing) {
        // 🛠️ FIX: On web, call getConfigData when first route is ready
        if (GetPlatform.isWeb &&
      ...
      // MODULE-FIRST ARCHITECTURE: Always start with Splash screen
