...
class _MenuScreenState extends State<MenuScreen> {
  ...
  Future<void> loadData(context) async {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (AuthHelper.isLoggedIn()) {
        debugPrint(
            '🔄 MenuScreen: Loading delegate, deliveryman, marketer, and store registration data...');
          ...
          return GetBuilder<KaidhaSubscriptionController>(builder: (kaidha) {
            return GetBuilder<Delegate_Controller>(
              ...
              return GetBuilder<DeliverymanRegistrationController>(
                ...
                // مندوب تسويق قسائم شرائية — MOVED OUT into the standalone
                // "مسوق شله" app (com.food.shala.marketer). Removed from the
                // customer app so the two are fully separate (Naser 2026-09-14).
                // The marketer build (APP_MODE=marketer) boots straight into the
                // service via DashboardScreen, so it doesn't need this menu entry.
                // To restore in the customer app, un-comment the block below.
                //
                // promoRows.add(GetBuilder<MarketerController>(
                //   builder: (marketerCtrl) {
                ...
                //       onTap: () => _runWithLoginRequired(
                //           () => Get.toNamed(RouteHelper.getMarketerRoute())),
                //     );
                //   },
                // ));
                if (showStore) {
        ...
        AlertDialog(
          ...
          actions: <Widget>[
            TextButton(
              onPressed: () async {
                Get.back();
                const String url = 'https://play.google.com/store/apps/details?id=com.delivery.shala&hl=en';
  ...
  // Kept for the commented-out marketer menu entry (moved to the standalone
  // "مسوق شله" app); unreferenced in the customer app until that is restored.
  // ignore: unused_element
  String _getDelegateSuffix(bool isLoggedIn, Delegate_Controller controller) {
    if (!isLoggedIn) return '';
    if (Get.isRegistered<MarketerController>()) {
      final mController = Get.find<MarketerController>();
      if (mController.data == null && mController.isLoading) {
        return '';
      }
