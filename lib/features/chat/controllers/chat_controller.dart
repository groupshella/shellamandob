...
class ChatController extends GetxController implements GetxService {
  ...
  StoreModel? _searchStoreModel;
  StoreModel? get searchStoreModel => _searchStoreModel;

  List<String> _chatSearchHistory = [];
  List<String> get chatSearchHistory => _chatSearchHistory;
  ...
  String? _currentPusherChannel;
  String? _currentPresenceChannel;
  ...
  Future<void> archiveConversation(int? conversationId) async {
    if (conversationId != null && _conversationModel?.conversations != null) {
      bool isSuccess = await chatServiceInterface.archiveConversation(conversationId);
      if (isSuccess) {
  ...
  void initPusher(int conversationId) {
    final String channelName = 'private-conversation.$conversationId';
    if (_currentPusherChannel != null && _currentPusherChannel != channelName) {
      unsubscribePusher();
    }
    _currentPusherChannel = channelName;
    ...
    final String presenceChannelName = 'presence-conversation.$conversationId';
    _currentPresenceChannel = presenceChannelName;
    pusher.subscribePresence(
      presenceChannelName,
      onSubscriptionSucceeded: (members) {
        if (kDebugMode) print('[CHAT] User joined presence channel: $presenceChannelName');
      },
      onMemberAdded: (member) {
        if (kDebugMode) print('[CHAT] Other member joined: $member');
      },
      onMemberRemoved: (member) {
        if (kDebugMode) print('[CHAT] Other member left: $member');
      },
    );
        ...
        try {
          ...
          if (decoded != null && decoded['message'] != null) {
            ...
            if (_messageModel != null) {
              ...
              final bool exists = messages.any((m) => m.id == newMessage.id);
              if (!exists) {
          ...
          if (kDebugMode) print('[CHAT] Error parsing NewMessageSent event: $e');
        }
      },
    );
    ...
    pusher.subscribePrivate(
      ...
      onEvent: (data) {
        if (_messageModel?.messages != null) {
          for (var m in _messageModel!.messages!) {
            m.isSeen = 1;
          }
          update();
        }
    ...
    // 4. ConversationTransferred / Status Changed
    pusher.subscribePrivate(
      channelName,
      eventName: 'ConversationTransferred',
      onEvent: (data) {
        try {
          final decoded = data is String ? jsonDecode(data) : data;
          if (decoded != null && decoded['conversation'] != null) {
            ...
            if (_messageModel?.conversation != null &&
                _messageModel!.conversation!.id == updatedConv.id) {
              _messageModel!.conversation!.status = updatedConv.status;
              _messageModel!.conversation!.closedAt = updatedConv.closedAt;
              _messageModel!.conversation!.isArchivedBySender =
                  updatedConv.isArchivedBySender;
              _messageModel!.conversation!.isArchivedByReceiver =
                  updatedConv.isArchivedByReceiver;
              update();
            }
            if (_conversationModel?.conversations != null) {
              final index = _conversationModel!.conversations!
                  .indexWhere((c) => c?.id == updatedConv.id);
              if (index != -1) {
                _conversationModel!.conversations![index] = updatedConv;
                update();
              }
            }
          }
        } catch (e) {
          if (kDebugMode) print('[CHAT] Error handling ConversationTransferred: $e');
        }
      },
    );

    // 5. ConversationUpdated
    pusher.subscribePrivate(
      channelName,
      ...
      onEvent: (data) {
        try {
          ...
          if (kDebugMode) print('[CHAT] Error handling ConversationUpdated: $e');
        }
      },
    );
  }
  ...
  void sendTypingEvent() {
    if (Get.isRegistered<PusherService>()) {
      if (_sendTypingTimer == null || !_sendTypingTimer!.isActive) {
        final pusher = Get.find<PusherService>();
        if (_currentPusherChannel != null) {
          pusher.triggerEvent(
            _currentPusherChannel!,
            'client-typing',
            {'user': 'customer'},
          );
        }
        if (_currentPresenceChannel != null) {
          pusher.triggerEvent(
            _currentPresenceChannel!,
            'client-typing',
            {'user': 'customer'},
          );
        }
  ...
  void unsubscribePusher() {
    if (_currentPusherChannel != null && Get.isRegistered<PusherService>()) {
      Get.find<PusherService>().unsubscribe(_currentPusherChannel!);
      _currentPusherChannel = null;
    }
    _typingTimer?.cancel();
  ...
  Future<void> searchConversation(String name) async {
    ...
    _searchTimer = Timer(const Duration(milliseconds: 500), () async {
      StoreModel? result = await chatServiceInterface.searchStoreList(name);
      if (result != null) {
        _searchStoreModel = result;
      } else {
        _searchStoreModel = StoreModel(totalSize: 0, offset: 1, stores: []);
      }
      _isSearchLoading = false;
      update();
    });
  ...
  void getChatSearchHistory() {
    _chatSearchHistory = chatServiceInterface.getChatSearchHistory();
    update();
  }

  void saveChatSearchHistory(String query) {
    final String q = query.trim();
    if (q.length < 2) return;
    _chatSearchHistory.remove(q);
    _chatSearchHistory.insert(0, q);
    if (_chatSearchHistory.length > 10) {
      _chatSearchHistory.removeRange(10, _chatSearchHistory.length);
    }
    chatServiceInterface.saveChatSearchHistory(_chatSearchHistory);
    update();
  }

  void clearChatSearchHistory() {
    chatServiceInterface.clearChatSearchHistory();
    _chatSearchHistory = [];
    update();
  }
  ...
  Future<void> getMessages(int offset, NotificationBodyModel? notificationBody,
    ...
    final int? actualConvId = _messageModel?.conversation?.id ?? effectiveConversationId;
    if (isSuccess && actualConvId != null && _messageModel != null) {
      initPusher(actualConvId);
    }
  ...
  Future<Response?> sendMessage(
    ...
    if (sendHttpSuccess) {
      ...
      if (sendBody is Map<String, dynamic>) {
        ...
        if (_messageModel?.conversation?.id != null) {
          initPusher(_messageModel!.conversation!.id!);
        }
      }
