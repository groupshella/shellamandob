...
class DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      GiftCampaignDeepLinkService.handlePendingGiftNavigation();
    });
  ...
  Widget build(BuildContext context) {
    // Standalone marketer app (APP_MODE=marketer): same login, then this build
    // opens directly on the marketer service instead of the customer dashboard.
    // Compile-time const → no effect on the normal customer build.
    if (AppConstants.isMarketerApp) {
      return const MarketerScreen();
    }
