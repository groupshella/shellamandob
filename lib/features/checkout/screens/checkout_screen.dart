...
class CheckoutScreenState extends State<CheckoutScreen>
  ...
  void initState() {
    ...
    Get.find<CheckoutController>().onAddressChanged =
        _onAddressChangedFromSheet;
  ...
  void _setSinglePaymentActive() {
    ...
    if ((!_firstTimeCheckPayment &&
      ...
      Future.delayed(const Duration(milliseconds: 600), () {
        ...
        Get.find<CheckoutController>()
            .changeDigitalPaymentName(paymentWay, willUpdate: false);
                  ...
                  ? () async {
                      ...
                      if (controller.isPaymentFlowInProgress) {
                        ...
                        if (!controller.isLoading) {
                          ...
                          showCustomSnackBar('pay_processing_order'.tr);
                      ...
                      try {
                        ...
                        if (selectedPaymentIndex == 0) {
                          ...
                          // Re-read qidhaBalance from freshly-fetched wallet or store-specific balance.
                          final int? checkoutStoreId = widget.storeId ??
                              (Get.isRegistered<StoreController>()
                                  ? Get.find<StoreController>().store?.id
                                  : null);
                          if (checkoutStoreId != null && checkoutStoreId != 1) {
                            final storeCtrl = Get.isRegistered<StoreController>()
                                ? Get.find<StoreController>()
                                : null;
                            final double? storeBal = storeCtrl?.store?.id == checkoutStoreId
                                ? (storeCtrl?.store?.qidhaAvailableBalance ?? storeCtrl?.store?.qidhaCreditLimit)
                                : kaidhaSubController.getStoreContractBalance(checkoutStoreId);
                            if (storeBal != null) {
                              qidhaBalance = storeBal;
                            } else {
                          ...
                          if (!kaidhaIsSigned) {
                            ...
                            Get.dialog(
                              AlertDialog(
                                ...
                                content: Text('pay_qidha_unsigned'.tr),
                                actions: [
                                  TextButton(
                                    onPressed: () => Get.back(),
                        ...
                        if (orderAmount <
                          ...
                          if (effectiveStoreId == null ||
                            ...
                            final CouponRevalidationResult couponRevalidation =
                              ...
                              cartSubtotal: Get.find<CartController>().subTotal,
                              currentModuleId:
                            ...
                            if (couponRevalidation !=
                              ...
                              showCustomSnackBar('pay_coupon_updated'.tr);
                            ...
                            final String? orderCouponTitle = hasOrderCoupon
                                ? (couponForOrder.coupon?.title != null &&
                                        couponForOrder.coupon!.title!.isNotEmpty
