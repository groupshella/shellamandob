...
class SearchScreenState extends State<SearchScreen>
  ...
  Future<void> _handleQueryClear() async {
    ...
    try {
      ...
      debugPrint(
          '[Search][QUERY_CLEAR] activeModuleId=${activeModuleId ?? 'null'}');
  ...
  void _switchTab(bool isStore) {
    ...
    if (searchController.isStore != isStore) {
      debugPrint('[Search][TAB] selected=${isStore ? 'stores' : 'items'}');
  ...
  Future<void> _initializeSearchStateOnOpen() async {
    ...
    final int initialVisualIndex = sortedModules.indexWhere(
        (module) => module.id != null && module.id == initialModule?.id);
