...
class BrandsController extends GetxController implements GetxService {
  ...
  Future<List<BrandModel>?> getBrandList(
    ...
    try {
      ...
      if (kDebugMode) {
        appLogger.error('Error checking module type in BrandsController: $e');
      }
    }
    ...
    if (businessSettings?.brandSection?.toString() == '1') {
      ...
      if (_brandList == null ||
        ...
        if (kDebugMode) {
          appLogger.debug(
              'BrandsController: Loading brands with dataSource = $dataSource');
        }
        ...
        if (!forceRefresh && dataSource != DataSourceEnum.client) {
          try {
            ...
            if (kDebugMode) {
              appLogger.warning(
                  'BrandsController: Error loading from comprehensive cache: $e');
            }
          }
        }
        ...
        if (dataSource == DataSourceEnum.local) {
          ...
          if (kDebugMode) {
            appLogger.debug(
                'BrandsController: Local brands loaded: ${brandList?.length ?? 0}');
          }
          ...
          if (kDebugMode) {
            appLogger.debug(
                'BrandsController: Client brands loaded: ${brandList?.length ?? 0}');
          }
          _prepareBandList(brandList);
        }
      } else {
        if (kDebugMode) {
          appLogger.debug(
              'BrandsController: Skipping load - brandList already populated and not forcing refresh');
        }
      }
    } else {
      if (kDebugMode) {
        appLogger.debug(
            'BrandsController: Brand section disabled, skipping brands loading');
      }
    }
    return _brandList;
  }
  ...
  Future<void> resetToDefault({bool force = false}) async {
    ...
    try {
      ...
      if (kDebugMode && isModuleSwitching) {
        debugPrint(
            '🔍„ BrandsController: Module switch detected ($_lastModuleId â†’ $currentModuleId), full reset');
      }
