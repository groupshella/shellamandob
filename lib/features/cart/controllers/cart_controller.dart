...
class CartController extends GetxController implements GetxService {
  ...
  Future<void> _persistCart() async {
    try {
      ...
      // ⚡ SYNC FIX: Persist to Hive cache immediately to keep it in sync with SharedPreferences/API
      final moduleId = _getModuleId();
      final cartListJson = _cartList.map((cart) => cart.toJson()).toList();
      await HiveHomeCacheService().saveCartData(moduleId, cartListJson);
      debugPrint(
          '💾 Persisted ${_cartList.length} items to SharedPreferences & Hive');
    } catch (e) {
  ...
  Future<void> addToCart(CartModel cartModel, int? index) async {
    ...
    if (kDebugMode) {
      ...
      debugPrint(
          '   - existingStoreId > 0: ${existingStoreId != null && existingStoreId > 0}');
      debugPrint('   - newStoreId != null: ${newStoreId != null}');
      debugPrint(
          '   - newStoreId > 0: ${newStoreId != null && newStoreId > 0}');
      debugPrint(
          '   - existingStoreId != newStoreId: ${existingStoreId != newStoreId}');
    ...
    if (kDebugMode) {
      debugPrint(
          '✅ addToCart: Same store or empty cart - adding item directly');
    }
    await _addToCartInternal(cartModel, index);
  }
  ...
  Future<bool> syncLocalCartRowsWithoutServerId() async {
    Future<bool> drainPendingOnce() async {
      ...
      while (iterations < maxIterations) {
        ...
        final OnlineCart? payload = _onlineCartFromCartModelForServerAdd(row);
        if (payload == null) {
          debugPrint(
              '⚠️ syncLocalCartRowsWithoutServerId: cannot build OnlineCart at index $idx');
          return false;
        }
        bool ok = await smartAddToCartOnline(payload, skipDebounce: true);
        if (!ok) {
          debugPrint(
          ...
          for (final dynamic o in vals) {
            if (o is Map && o['label'] != null) {
              options.add(VariationOption(
                ...
                optionPrice: double.tryParse('${o['optionPrice'] ?? 0}') ?? 0.0,
              ));
            }
          }
        }
  ...
  OnlineCart? _onlineCartFromCartModelForServerAdd(CartModel c) {
    ...
    final String unitPrice = (c.discountedPrice ?? c.price ?? 0.0).toString();
  ...
  Future<void> clearCartList({bool canRemoveOnline = true}) async {
    ...
    if ((AuthHelper.isLoggedIn() || AuthHelper.isGuestLoggedIn()) &&
        ...
        canRemoveOnline) {
      // 🔥 FIX: AWAIT the server clear. This used to be fire-and-forget
      // (`clearCartOnline();` with no await), so its response — which sets
      // _cartList = [] and calls _onCartMutated — could land AFTER a caller's
      // subsequent addToCartOnline() call finished. That silently wiped out
      // the item the user had just added and re-broadcast an empty cart,
      // which is why the badge briefly updated and then reset itself.
  ...
  bool existAnotherStoreItem(int? storeID, int? moduleId) {
    ...
    if (currentCartStoreId != null && currentCartStoreId > 0) {
      if (effectiveNewStoreId == null || effectiveNewStoreId <= 0) {
        if (kDebugMode) {
          debugPrint(
              '   -> New item storeID is null/invalid - blocking add (store must be known)');
        }
        return true;
      }
      ...
      if (effectiveNewStoreId != currentCartStoreId) {
        if (kDebugMode) {
          debugPrint(
              '   -> DIFFERENT STORE: $effectiveNewStoreId != $currentCartStoreId');
        }
        return true;
      }
      ...
      if (kDebugMode) {
        debugPrint(
            '   -> SAME STORE: $effectiveNewStoreId == $currentCartStoreId');
      }
      return false;
    }
  ...
  Future<bool> smartAddToCartOnline(
    ...
    if (existingIndex != -1) {
      ...
      debugPrint(
          '➕ Existing item in same store, using ADD API to append quantity');
  ...
  Future<bool> addToCartWithFallback({
    ...
    if (_cartList.isNotEmpty &&
        (effectiveStoreId == null || effectiveStoreId <= 0)) {
      debugPrint(
    ...
    try {
      ...
      if (onlineSuccess) {
        ...
        if (isFinalBusinessRejection) {
          ...
          _onCartMutated(
              reason: 'addToCartWithFallback_businessRejectRollback');
          await cartServiceInterface.addSharedPrefCartList(_cartList);
          try {
            final moduleId = ModuleHelper.getCacheModule()?.id;
            final cartListJson =
                _cartList.map((cart) => cart.toJson()).toList();
            await HiveHomeCacheService().saveCartData(moduleId, cartListJson);
          } catch (e) {
            debugPrint(
                '⚠️ addToCartWithFallback: rollback Hive save failed: $e');
          }
          try {
            await getCartDataOnline(forceRefresh: true);
          } catch (e) {
  ...
  OnlineCart _buildOnlineCartWithResolvedIds(
    ...
    int index =
        existingIndex ?? _findExistingCartItem(cart.itemId, cart.variant);
    if (index == -1) {
      index = _findExistingCartItemByItemOnly(cart.itemId);
    }
    final int? finalCartId =
    ...
    if ((resolvedStoreId == null || resolvedStoreId <= 0) && index != -1) {
      resolvedStoreId =
          _cartList[index].storeId ?? _cartList[index].item?.storeId;
    }
  ...
  Future<void> getCartDataOnline({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (onlineCartList != null && onlineCartList.isNotEmpty) {
        ...
        _isCartCleared = false;
        final int? newStoreIdForCoupon = _storeId ??
            (_cartList.isNotEmpty ? _cartList.first.item?.storeId : null);
        if (Get.isRegistered<CouponController>()) {
  ...
  Future<void> _clearLocalCartItems() async {
    try {
      ...
      // ⚡ SYNC FIX: Clear Hive cache as well to prevent old items from being restored
      final moduleId = _getModuleId();
      await HiveHomeCacheService()
          .saveCartData(moduleId, <Map<String, dynamic>>[]);
      debugPrint('🧹 Cleared SharedPreferences & Hive local cart items');
    } catch (e) {
  ...
  int? _getModuleId() {
    return ModuleHelper.getModule()?.id ?? ModuleHelper.getCacheModule()?.id;
  }
}
