...
class KaidhaSubService implements kaidhaSub_ServiceInterface {
  ...
  @override
  Future<NafathCheckStatusModel?> Nafath_send_checkStatus(BuildContext context, String nationalId, {bool confirmed = false}) async {
    return await kaidhaSubRepositoryinterface.Nafath_send_checkStatus(context, nationalId, confirmed: confirmed);
  }
  ...
  @override
  Future<Response> sendStoreContractRequest({
    required int storeId,
    required double amount,
    String? notes,
  }) async {
    return await kaidhaSubRepositoryinterface.sendStoreContractRequest(
      storeId: storeId,
      amount: amount,
      notes: notes,
    );
  ...
  Future<List<int>> getRequestedStoreIds() async {
    return await kaidhaSubRepositoryinterface.getRequestedStoreIds();
  }

  @override
  Future<List<Store>?> getQidhaContractedStores({int? moduleId, String? type, String? search, int? offset, int? limit}) async {
    return await kaidhaSubRepositoryinterface.getQidhaContractedStores(
      moduleId: moduleId,
      type: type,
      search: search,
      offset: offset,
  ...
  Future<List<Store>?> getQidhaAvailableContractStores({int? moduleId, String? type, String? search, int? offset, int? limit}) async {
    return await kaidhaSubRepositoryinterface.getQidhaAvailableContractStores(
      moduleId: moduleId,
      type: type,
      search: search,
      offset: offset,
