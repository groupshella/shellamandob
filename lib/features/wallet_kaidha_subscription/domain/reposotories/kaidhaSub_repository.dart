...
class KaidhaSubRepository implements KaidhaSubRepositoryInterface {
  ...
  Future<NafathCheckStatusModel?> Nafath_send_checkStatus(
      BuildContext context, String nationalId, {bool confirmed = false}) async {
    final Map<String, dynamic>? data = await _buildNafathBody(nationalId);
    if (data == null) return null;
    // `confirmed` is only true when the user explicitly taps "تحقق من الحالة"
    // (i.e. after approving in Nafath). Silent background polls omit it so the
    // request stays pending until the user actually confirms.
  ...
  @override
  Future<Response<dynamic>> sendStoreContractRequest({
    ...
    if (kDebugMode) {
      debugPrint('📤 [QidhaStoreContract] Sending request: $data');
    }

    final Response<dynamic> response = await apiClient.postData(
      AppConstants.storeContractRequestUri,
      data,
      headers: const {
        'Accept': 'application/json',
      },
    ...
    if (kDebugMode) {
      debugPrint(
          '📥 [QidhaStoreContract] Response code: ${response.statusCode}, body: ${response.body}');
    }
  ...
  @override
  Future<List<int>> getRequestedStoreIds() async {
    try {
      final Response<dynamic> response = await apiClient.getData(
        AppConstants.getStoreContractRequestsUri,
        headers: const {
          'Accept': 'application/json',
        },
      );
      ...
      if (response.statusCode == 200 && response.body != null) {
        ...
        if (body is Map && body['requested_store_ids'] is List) {
          final List<dynamic> list = body['requested_store_ids'] as List<dynamic>;
          return list.map((e) => int.tryParse(e.toString()) ?? 0).where((id) => id > 0).toList();
        }
      }
    } catch (e) {
      if (kDebugMode) {
        debugPrint('⚠️ [QidhaStoreContract] Error fetching requested store IDs: $e');
      }
    }
    return [];
  }
  ...
  @override
  Future<List<Store>?> getQidhaContractedStores({int? moduleId, String? type, String? search, int? offset, int? limit}) async {
    try {
      String uri = AppConstants.qidhaContractedStoresUri;
      final List<String> params = [];
      if (moduleId != null) params.add('module_id=$moduleId');
      ...
      final Response<dynamic> response = await apiClient.getData(
        uri,
        headers: const {'Accept': 'application/json', 'X-Disable-ETag': 'true'},
        useEtag: false,
      );
      ...
      if (response.statusCode == 200 && response.body != null) {
        ...
        if (body is Map && body['stores'] is List) {
          list = body['stores'] as List<dynamic>;
      ...
      if (kDebugMode) {
        debugPrint('⚠️ [QidhaContractedStores] Error fetching contracted stores: $e');
      }
    }
    return null;
  }
  ...
  @override
  Future<List<Store>?> getQidhaAvailableContractStores({int? moduleId, String? type, String? search, int? offset, int? limit}) async {
    try {
      String uri = AppConstants.qidhaAvailableContractStoresUri;
      final List<String> params = [];
      if (moduleId != null) params.add('module_id=$moduleId');
      if (type != null) params.add('type=$type');
      if (search != null && search.isNotEmpty) params.add('search=${Uri.encodeComponent(search)}');
      ...
      if (kDebugMode) {
        debugPrint('⚠️ [QidhaAvailableContractStores] Error fetching available contract stores: $e');
      }
    }
    return null;
  }
