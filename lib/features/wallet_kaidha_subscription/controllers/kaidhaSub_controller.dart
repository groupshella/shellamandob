...
class KaidhaSubscriptionController extends GetxController
  ...
  void syncFullNameToParts() {
    final parts = fullNameController.text.trim().split(RegExp(r'\s+'));
    if (parts.isNotEmpty) firstname.text = parts[0];
    if (parts.length > 1) fathername.text = parts[1];
    if (parts.length > 2) grandfathername.text = parts[2];
    if (parts.length > 3) {
      last_name.text = parts.sublist(3).join(' ');
    } else if (parts.length == 3) {
      last_name.text = parts[2];
    }
  }
  ...
  bool hasAgreedToContract = false;

  void setHasAgreedToContract(bool value) {
    hasAgreedToContract = value;
    update();
  }
  List<NamedFile> personalDocuments = [];
  List<NamedFile> incomeDocuments = [];

  Future<void> pickDocuments({required bool isIncome}) async {
    try {
      ...
      if (result != null) {
        for (var file in result.files) {
          if (file.size <= 5 * 1024 * 1024) {
            if (isIncome) {
              incomeDocuments.add(NamedFile(name: file.name, file: file));
            } else {
  ...
  void removeDocument(int index, {required bool isIncome}) {
    if (isIncome) {
      incomeDocuments.removeAt(index);
    } else {
      personalDocuments.removeAt(index);
    }
    update();
  }

  bool isAttachedDocumentsEmpty = false;
  ...
  Future<NafathCheckStatusModel?> Nafath_send_checkStatus(
    ...
    // `confirmed` = user explicitly tapped "تحقق من الحالة" after approving in
    // Nafath (silent==false). Silent background polls leave it false so the
    // request only advances to review once the customer actually confirms.
    final onValue = await kaidhaSubServiceInterface.Nafath_send_checkStatus(
        context, nationalId, confirmed: !silent);
    if (!context.mounted) {
      return null;
    }
    ...
    if (onValue != null) {
      ...
      if (onValue.status == 'approved') {
        ...
        _nafath_checkStatus = NafathCheckStatusModel(
          ...
          createdAt: onValue.createdAt,
          random: onValue.random ??
              int.tryParse(_cachedNafathRandomCode ?? '') ??
              int.tryParse(_nafath_national_id?.code ?? ''),
          fullNameAr: onValue.fullNameAr,
          signedFileUrl: onValue.signedFileUrl,
        );
        _isShow = false;
...
//  ----------------------------------------------------------------------------
  ...
  bool validate_Fields_Screen_1(BuildContext context) {
    ...
    isFullNameEmpty = fullNameController.text.trim().isEmpty && firstname.text.trim().isEmpty;
    isIdentityCardInvalid = false;
    clearFieldErrors();
  ...
  Future<bool> validate_Fields_Screen_Combined(BuildContext context) async {
    bool ok1 = validate_Fields_Screen_1(context);
    if (!ok1) return false;
    return await validate_Fields_Screen_2(context);
  }

  Future<bool> validate_Fields_Screen_2(BuildContext context, [String? nationalId, bool submit = true]) async {
    ...
    try {
      total_salary.text = monthlyIncome.text;

      All_files.clear();
      All_files.addAll(personalDocuments);
      All_files.addAll(incomeDocuments);
  ...
  // ── Store Contract Requests & Discovery ─────────────────────────────────
  final Set<int> _requestedStoreIds = <int>{};
  Set<int> get requestedStoreIds => _requestedStoreIds;

  bool _isSendingContractRequest = false;
  bool get isSendingContractRequest => _isSendingContractRequest;

  List<Store>? _qidhaStoresList;
  List<Store>? get qidhaStoresList => _qidhaStoresList;

  bool _isLoadingQidhaStores = false;
  bool get isLoadingQidhaStores => _isLoadingQidhaStores;

  int _qidhaStoresOffset = 1;
  int get qidhaStoresOffset => _qidhaStoresOffset;

  bool _qidhaStoresHasMore = true;
  bool get qidhaStoresHasMore => _qidhaStoresHasMore;

  bool _isLoadingMoreQidhaStores = false;
  bool get isLoadingMoreQidhaStores => _isLoadingMoreQidhaStores;

  final Map<int, double> _contractedStoreBalances = {};
  final Map<int, double> _contractedStoreLimits = {};
  Map<int, double> get contractedStoreBalances => _contractedStoreBalances;
  Map<int, double> get contractedStoreLimits => _contractedStoreLimits;

  double? getStoreContractBalance(int? storeId) {
    if (storeId == null) return null;
    return _contractedStoreBalances[storeId];
  }

  double? getStoreContractLimit(int? storeId) {
    if (storeId == null) return null;
    return _contractedStoreLimits[storeId];
  }

  Future<void> loadRequestedStoreIds() async {
    final ids = await kaidhaSubServiceInterface.getRequestedStoreIds();
    _requestedStoreIds.addAll(ids);
    update();
  }

  Future<void> getQidhaStores({
    required bool? isContractedOnly,
    int? moduleId,
    String? type,
    String? search,
    int offset = 1,
    ...
    if (offset == 1) {
      _isLoadingQidhaStores = true;
      _qidhaStoresOffset = 1;
      _qidhaStoresHasMore = true;
      if (reload) {
        _qidhaStoresList = null;
      }
      update();
    } else {
      if (!_qidhaStoresHasMore || _isLoadingMoreQidhaStores) return;
      _isLoadingMoreQidhaStores = true;
      update();
    }

    try {
      List<Store>? stores;
      if (isContractedOnly == true) {
        stores = await kaidhaSubServiceInterface.getQidhaContractedStores(
          moduleId: moduleId,
          type: type,
          search: search,
          offset: offset,
      ...
      } else if (isContractedOnly == false) {
        stores = await kaidhaSubServiceInterface.getQidhaAvailableContractStores(
          moduleId: moduleId,
          type: type,
          search: search,
          offset: offset,
        ...
        // Fetch all: get contracted and available
        final contracted = await kaidhaSubServiceInterface.getQidhaContractedStores(
          moduleId: moduleId,
          type: type,
          search: search,
          offset: offset,
        ...
        final available = await kaidhaSubServiceInterface.getQidhaAvailableContractStores(
          moduleId: moduleId,
          type: type,
          search: search,
          offset: offset,
        ...
        stores = [
          if (contracted != null) ...contracted,
          if (available != null) ...available,
        ];
      }

      if (stores == null || stores.length < limit) {
        _qidhaStoresHasMore = false;
      }

      if (stores != null) {
        for (final s in stores) {
          if (s.id != null && (s.qidhaAvailableBalance != null || s.qidhaCreditLimit != null)) {
            _contractedStoreBalances[s.id!] = s.qidhaAvailableBalance ?? s.qidhaCreditLimit ?? 0.0;
            _contractedStoreLimits[s.id!] = s.qidhaCreditLimit ?? 0.0;
          }
        }
      }

      if (offset == 1) {
        _qidhaStoresList = stores ?? [];
      } else {
        _qidhaStoresList ??= [];
        if (stores != null && stores.isNotEmpty) {
          final existingIds = _qidhaStoresList!.map((s) => s.id).toSet();
          for (final s in stores) {
            if (!existingIds.contains(s.id)) {
              _qidhaStoresList!.add(s);
            }
          }
        }
      }

      _qidhaStoresOffset = offset;
      _isLoadingQidhaStores = false;
      _isLoadingMoreQidhaStores = false;
      update();
    } catch (e) {
      _isLoadingQidhaStores = false;
      _isLoadingMoreQidhaStores = false;
  ...
  Future<void> loadMoreQidhaStores({
    required bool? isContractedOnly,
    int? moduleId,
    String? type,
    String? search,
  }) async {
    if (!_qidhaStoresHasMore || _isLoadingMoreQidhaStores || _isLoadingQidhaStores) {
      return;
    }
    await getQidhaStores(
      isContractedOnly: isContractedOnly,
      moduleId: moduleId,
      type: type,
      search: search,
      offset: _qidhaStoresOffset + 1,
    );
  }

  Future<bool> sendStoreContractRequest({
    required int storeId,
    required double amount,
    String? notes,
  }) async {
    _isSendingContractRequest = true;
    update();

    try {
      final response = await kaidhaSubServiceInterface.sendStoreContractRequest(
        storeId: storeId,
        amount: amount,
        notes: notes,
      );
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        _requestedStoreIds.add(storeId);
        update();
        return true;
      } else {
        final message = response.body is Map
            ...
            : null;
        showCustomSnackBar(message ?? 'فشل إرسال طلب التعاقد، يرجى المحاولة لاحقاً');
        update();
        return false;
      }
    } catch (e) {
