...
import 'package:sixam_mart/features/order/widgets/invoice_bottom_sheet.dart';
...
class OrderTrackingScreenState extends State<OrderTrackingScreen> {
  Timer? _timer;
  // Instant refresh on a push (order-status / driver update) — complements the
  // 8s poll so the map + steps update the moment an FCM message arrives.
  ...
  bool _isOrderInTerminalState(OrderModel? order) {
    ...
    // NOTE: 'paid' is intentionally NOT terminal here — a prepaid order (card /
    // wallet / Apple Pay) is 'paid' immediately but is still being delivered, so
    // treating 'paid' as terminal used to cancel the live-tracking poll on the
    // first tick (frozen driver + steps). Only genuinely dead payment states stop
    // it; delivery completion is driven by the order status below.
  ...
  // Refresh the tracked order the instant a foreground push arrives. Any push
  // while this screen is current triggers a fresh trackOrder fetch; the
  // controller's own in-flight guard prevents overlap with the 8s poll. This
  // gives near-instant step/driver updates without waiting for the next tick.
  void _listenForPushUpdates() {
    _fcmSub = FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      if (!mounted) return;
      final OrderController orderController = Get.find<OrderController>();
      if (_isOrderInTerminalState(orderController.trackModel)) return;
      orderController.timerTrackOrder(widget.orderID.toString(),
          contactNumber: widget.contactNumber);
    });
  }
