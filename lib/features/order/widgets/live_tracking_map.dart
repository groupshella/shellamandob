...
class _LiveTrackingMapState extends State<LiveTrackingMap>
  ...
  // Routes API key (Google Routes API v2). Map tiles keep using the key in
  // AndroidManifest / AppDelegate; this key is used ONLY for the route request
  // and must have the Routes API + Billing enabled on its project. Overridable
  // via --dart-define=GOOGLE_MAPS_KEY=...
  static const String _routesApiKey =
      'AIzaSyDwpl1O5yMBvB9JHtZz61I3P3uz_ClvXP8';
  ...
  /// Direct call to the Google Routes API v2 (computeRoutes) — the modern
  /// replacement for the legacy Directions API. Returns an ENCODED polyline
  /// decoded the same way (flutter_polyline_points), so the route follows the
  /// real roads. Returns [] on any failure so the caller keeps its fallback.
  Future<List<LatLng>> _fetchDirectionsDirect(
    ...
    if (res.statusCode != 200) return const []; // e.g. 403 API disabled/billing
    final data = jsonDecode(res.body) as Map<String, dynamic>;
    final routes = data['routes'] as List?;
    if (routes == null || routes.isEmpty) return const [];
    final encoded = (routes.first as Map<String, dynamic>)['polyline']
