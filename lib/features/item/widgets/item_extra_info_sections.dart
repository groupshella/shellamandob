...
class ItemExtraInfoSections extends StatelessWidget {
  ...
  /// Arabic). A mostly-Arabic description containing a few English words/brand
  /// names is NOT flagged.
  bool _isPredominantlyEnglish(String text) {
  ...
  Widget _description() {
    final String? desc = controllerItem?.description;
    final bool hasData = show && desc != null && desc.isNotEmpty;
    if (!hasData) return const SizedBox();
    // In the Arabic UI, don't show English (or English-dominant) descriptions —
    // we hide untranslated English copy rather than display it to Arabic users.
    if (_isArabicLocale() && _isPredominantlyEnglish(desc)) {
