...
class _HomeSearchScreenState extends State<HomeSearchScreen> {
  ...
  /// Cross-module product search via `/api/v1/items/search` (no module_id â‡’
  ...
  Future<void> _runSearch(String text) async {
    ...
    try {
      if (Get.isRegistered<ApiClient>()) {
        ...
        // Fire request (single if crossModule, scoped if not)
    ...
    if (q.length >= 2) {
      results =
          results.where((p) => _nameMatchesQuery(p.name ?? '', q)).toList();
    }
    ...
    int firstWordRank(_SearchProduct p) {
      ...
      final int idx = n.indexOf(RegExp(r'[^\s\-/,،()\[\].]'));
      if (idx >= 0 && arabicDedupKey(n.substring(idx)).startsWith(q)) return 0;
      return 1;
    }
    ...
    for (final storeId in toFetch) {
      try {
        ...
        final String rawLogo =
            (store['logo_full_url']?.toString() ?? '').isNotEmpty
  ...
  // â”€â”€ Arabic text normalization (delegates to the testable util) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  String _stripMarks(String s) => stripArabicMarks(s);
  String _normCore(String s) => normalizeArabic(s);

  String _getStoreTypeName(String? moduleType) {
    if (moduleType == null) return '';
    final String t = moduleType.toLowerCase().trim();
    final bool isArabic = Get.locale?.languageCode == 'ar';
    if (t == 'food') {
      return isArabic ? 'مطعم' : 'Restaurant';
    } else if (t == 'coffee' || t == 'cafe') {
      return isArabic ? 'مقهى' : 'Café';
    } else if (t == 'ecommerce' || t == 'grocery') {
      return isArabic ? 'هايبر ماركت' : 'Hypermarket';
    } else if (t == 'pharmacy') {
  ...
  bool _nameMatchesQuery(String name, String normQ) {
    if (arabicDedupKey(name).startsWith(normQ)) return true;
    final RegExp boundary = RegExp(r'[\s\-/,،()\[\].]+');
    for (final m in boundary.allMatches(name)) {
      final String substring = name.substring(m.end);
      if (substring.trim().isEmpty) continue;
      if (arabicDedupKey(substring).startsWith(normQ)) return true;
    }
    return false;
  }
  ...
  Future<void> _fetchModuleScoped() async {
    ...
    try {
      ...
      // terms. Over-fetch (20) because cleaning/de-dup trims to â‰¤10.
      final int? mid = widget.moduleId;
      final r = await api.getData(
    ...
    try {
      ...
      final List<int?> railModuleIds =
          widget.moduleId != null ? <int?>[widget.moduleId] : _allModuleIds();
      ...
      for (final int? rmid in railModuleIds) {
        try {
          final r = await api.getData(
            ...
            useEtag: false,
            headers:
                rmid != null ? {AppConstants.moduleId: rmid.toString()} : null,
          );
          final dynamic body = r.body;
      ...
      if (mounted) {
        ...
        for (final m in storeList) {
          ...
          final String? logo = (m['logo_full_url'] ?? m['logo'])?.toString();
        ...
        // «الأكثر بحثاً» = this section's STORE names (e.g. restaurants), up to
        ...
        child: onDelete == null
            ...
            : Row(
                ...
                children: [
                  ...
                  // Per-chip delete (أ—) for recent searches.
                  InkWell(
                    onTap: onDelete,
  ...
  // Removes a single recent-search term (أ— on the chip).
  ...
  // â”€â”€ Suggestions (shown while debounce is running / before first search) â”€â”€
  ...
  Widget _buildResults() {
    ...
    final List<_SearchProduct> productList =
        isOffersTab ? _results.where((p) => p.discount > 0).toList() : _results;
    ...
    return ListView(
      ...
      children: [
        ...
        if (showStores)
          for (final s in storesToShow) _storeMatchCard(s),
        if (showProducts)
  ...
  Widget _storeGroup(List<_SearchProduct> items) {
    ...
    final double? distance =
        branches.isNotEmpty ? branches.first.distance : null;
        ...
        children: [
          ...
          GridView.builder(
            ...
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              ...
              // Card ratio 104 أ— 130 from the design.
  ...
  Widget _productCard(_SearchProduct item) {
    ...
    final bool isProductAvailable = item.item == null ||
        ((item.item!.stock == null || item.item!.stock! > 0) &&
            DateConverter.isAvailable(
                item.item!.availableTimeStarts, item.item!.availableTimeEnds));
...
class _SearchProduct {
  ...
  factory _SearchProduct.fromJson(Map<String, dynamic> j) {
    Item? parsedItem;
    try {
      parsedItem = Item.fromJson(j);
    } catch (e) {
      debugPrint('Error parsing Item in _SearchProduct: $e');
    }
    return _SearchProduct(
      id: _toInt(j['id']),
      name: j['name']?.toString(),
      imageUrl: _resolveUrl((j['image_full_url'] ?? j['image'])?.toString()),
