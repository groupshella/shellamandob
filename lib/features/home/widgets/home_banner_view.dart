...
class HomeBannerView extends StatefulWidget {
  ...
  static const double _aspectRatio = 613 / 250;
...
class _HomeBannerViewState extends State<HomeBannerView> {
  ...
  Future<void> _fetch() async {
    ...
    if (address == null ||
        ...
        address.zoneIds!.isEmpty) {
      if (kDebugMode) {
        debugPrint('   Skip API fetch - zone/location headers not ready');
      }
      if (mounted) setState(() => _loading = false);
      return;
    }
    ...
    try {
      // Fetch normal banners for the currently selected module.
      // The backend handles the fallback for Module 6 -> Module 3 automatically.
      final splashController = Get.find<SplashController>();
      final String moduleId = splashController.module?.id?.toString() ?? '3';

      final response = await Get.find<ApiClient>().getData(
        AppConstants.bannerUri, // Removed ?featured=1
        headers: {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<LocationController>(
      builder: (locationController) {
        final hasZone =
            AddressHelper.getUserAddressFromSharedPref()?.zoneIds?.isNotEmpty ??
                false;

        if (hasZone && _images.isEmpty && !_loading) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted && _images.isEmpty && !_loading) {
              setState(() {
                _loading = true;
              });
              _fetch();
