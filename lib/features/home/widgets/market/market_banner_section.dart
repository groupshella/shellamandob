...
class _MarketBannerSectionState extends State<MarketBannerSection> {
  ...
  @override
  void didUpdateWidget(covariant MarketBannerSection oldWidget) {
  ...
  Future<void> _fetch() async {
    ...
    try {
      final int targetModuleId = widget.moduleId ?? 3;
      final response = await Get.find<ApiClient>().getData(
        ...
        headers: {
          ...
          AppConstants.moduleId: targetModuleId.toString(),
        },
        useEtag: false,
      );
      if (!mounted) return;
      final dynamic body = response.body;
      // Merge module banners + campaigns; tolerate a bare list too.
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<LocationController>(
      builder: (locationController) {
        final hasZone = AddressHelper.getUserAddressFromSharedPref()?.zoneIds?.isNotEmpty ?? false;
