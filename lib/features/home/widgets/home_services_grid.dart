...
class HomeServicesGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<SplashController>(builder: (splash) {
      final List<ModuleModel> modules = (splash.moduleList ?? const [])
          .where((m) =>
              (m.moduleType ?? '').toLowerCase() != AppConstants.taxi &&
              (m.moduleType ?? '').toLowerCase() != 'rental' &&
              m.id != 10)
          .toList();
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Expanded(
                    child: _buildColumn(context, right, isRightColumn: true)),
                const SizedBox(width: _gap),
                Expanded(
                    child: _buildColumn(context, left, isRightColumn: false)),
              ],
            ),
          ],
        ),
  ...
  Widget _buildColumn(BuildContext context, List<ModuleModel> modules,
    ...
    final double height = isRightColumn ? 82.0 : 50.0;
    return Column(
      children: [
