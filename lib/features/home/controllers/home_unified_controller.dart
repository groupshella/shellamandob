...
class HomeUnifiedController extends GetxController implements GetxService {
  ...
  Future<bool> loadHomeData({
    ...
    try {
      ...
      if (address == null || address.zoneIds == null || address.zoneIds!.isEmpty) {
        if (kDebugMode) {
          appLogger.warning('   Skip API fetch - zone/location headers not ready');
        }
        _isLoading = false;
        return hasCachedData;
      }
  ...
  Future<bool> preloadModuleDataForSplash(
    ...
    try {
      ...
      if (address == null || address.zoneIds == null || address.zoneIds!.isEmpty) {
        if (kDebugMode) {
          appLogger.warning('   Skip splash preload API fetch - zone/location headers not ready');
        }
        return false;
      }
  ...
  void _refreshFromApiInBackground(int moduleId) {
    ...
    Future.delayed(const Duration(milliseconds: 100), () async {
      ...
      try {
        ...
        if (address == null || address.zoneIds == null || address.zoneIds!.isEmpty) {
          if (kDebugMode) {
            appLogger.warning('[Cache-First] Background refresh skipped - zone/location headers not ready');
          }
          return;
        }
