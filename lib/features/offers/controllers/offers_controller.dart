...
class OffersController extends GetxController implements GetxService {
  ...
  Future<OffersModel?> getOffers({int? specificModuleId}) async {
    ...
    if (addressModel == null || addressModel.zoneIds == null || addressModel.zoneIds!.isEmpty) {
      if (kDebugMode) {
        debugPrint(
            '⏭️ OffersController.getOffers: Skip API fetch - zone/location headers not ready');
      }
      return offersMode;
    }
  ...
  Future<void> getAggregatedOffers(List<int> moduleIds) async {
    ...
    if (addressModel == null || addressModel.zoneIds == null || addressModel.zoneIds!.isEmpty) {
      if (kDebugMode) {
        debugPrint(
            '⏭️ OffersController.getAggregatedOffers: Skip load - zone/location headers not ready');
      }
      return;
    }
