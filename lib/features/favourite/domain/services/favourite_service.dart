...
class FavouriteService implements FavouriteServiceInterface {
  ...
  @override
  Future<ResponseModel> addFavouriteList(int? id, bool isStore, {bool isOrder = false}) async {
    final result = await favouriteRepositoryInterface.add(null, isStore: isStore, isOrder: isOrder, id: id);
    return result is ResponseModel ? result : ResponseModel(false, 'Error');
  }

  @override
  Future<ResponseModel> removeFavouriteList(int? id, bool isStore, {bool isOrder = false}) async {
    final result = await favouriteRepositoryInterface.delete(id, isStore: isStore, isOrder: isOrder);
    return result is ResponseModel ? result : ResponseModel(false, 'Error');
  }
