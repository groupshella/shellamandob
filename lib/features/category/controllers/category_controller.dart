...
class CategoryController extends GetxController implements GetxService {
  ...
  void debugPrintCategoryIndex({String source = 'unknown'}) {
    ...
    if (list == null || list.isEmpty) {
      debugPrint('📱š Category index [$source]: empty');
      return;
    }
    debugPrint('📱š Category index [$source]: ${list.length} categories');
  ...
  void loadMoreCategories() {
    ...
    if (currentList != null && visibleCount < currentList.length) {
      ...
      if (kDebugMode) {
        debugPrint('🧠© Categories visible: $visibleCount / ${currentList.length}');
      }
  ...
  Future<void> clearCacheForCategory(int? categoryId) async {
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('⚠️ CategoryController.clearCacheForCategory failed: $e');
      }
    }
  }
  ...
  Future<List<CategoryModel>?> getCategoryList(bool reload,
    ...
    if (_categoryList == null ||
      ...
      try {
        ...
        debugPrint('⚠️ CategoryController: HomeController not available: $e');
      }
  ...
  void _prepareCategoryList(List<CategoryModel>? categoryList,
    ...
    if (categoryList != null) {
      ...
      if (categoryResults.isNotEmpty) {
        ...
        debugPrint('🔍 CategoryController: Sample filtered categories (first 3):');
        final sampleCount =
  ...
  void getSubCategoryList(String? categoryID,
    ...
    if (kDebugMode) {
      debugPrint(
          '📱ž CategoryController.getSubCategoryList ENTRY: categoryID=$categoryID');
    }
    ...
    if (kDebugMode && !shouldFetchItems) {
      debugPrint(
          '✅ CategoryController.getSubCategoryList: Item fetch skipped (store mode / cuisine category)');
    }
  ...
  void getCategoryItemList(
    ...
    // ✅ Only block items while the stores tab is active.
    if (_isStore && !allowWhenStore) {
      if (kDebugMode) {
        debugPrint(
            '🚫 CategoryController: Skipping getCategoryItemList - store tab active (categoryId: $categoryID)');
      }
      return;
    }
        ...
        if (categoryItem.items != null && categoryItem.items!.isNotEmpty) {
          ...
          if (kDebugMode) {
            ...
            for (final item in newItems) {
              debugPrint('   ✓ [ID: ${item.id}] ${item.name}');
            }
          }
          ...
          if (kDebugMode) {
            debugPrint(
                '📱Š [Page $effectiveOffset] Total items in list: ${_categoryItemList!.length}');
          }
